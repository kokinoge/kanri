...
export async function POST(request: NextRequest) {
  try {
    console.log('[IMPORT_DEPARTMENT_PERFORMANCE] Import request received');
    ...
    console.log('[IMPORT_DEPARTMENT_PERFORMANCE] CSV headers:', headers);

    // 必須ヘッダーの確認
    const requiredHeaders = ['department', 'clientName'];
      ...
      try {
        ...
        if (!client) {
          client = await prisma.client.create({
            data: {
              name: row.clientName,
              businessDivision: row.department,
              salesDepartment: row.salesDepartment || 'その他',
              priority: row.clientPriority || 'C'
            }
          });
        } else {
          // 既存クライアントの事業部を更新
          await prisma.client.update({
            ...
            data: {
              businessDivision: row.department,
              salesDepartment: row.salesDepartment || client.salesDepartment,
              priority: row.clientPriority || client.priority
            }
          });
        }
          ...
          if (!campaign) {
            campaign = await prisma.campaign.create({
              data: {
                ...
                purpose: '',
                totalBudget: row.campaignTotalBudget ? parseFloat(row.campaignTotalBudget) : 0,
        ...
        console.error(`[IMPORT_DEPARTMENT_PERFORMANCE] ${errorMsg}`);
      }
    }

    console.log(`[IMPORT_DEPARTMENT_PERFORMANCE] Import completed: ${imported} imported, ${errors} errors`);
    ...
    console.error('[IMPORT_DEPARTMENT_PERFORMANCE] Import error:', error);
    return NextResponse.json(
      { 
